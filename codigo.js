const fs = require('fs');
const { routeExists, isDirectory, isMarkdownFile, exploreDirectory, extractLinksFromFileMD, validateLinks, calculateStats } = require('./function.js');
    ...
    if (isDirectory(ruta)) {
      ...
      if (options.validate) {
        ...
        Promise.all(validatePromises)
          .then((validatedLinks) => {
            if (options.stats) { // Verificar la op stats 
              const stats = calculateStats(validateLinks); 
              resolve(stats);
            } else {
